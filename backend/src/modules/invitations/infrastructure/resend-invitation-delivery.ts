...
export class ResendInvitationDelivery extends InvitationDelivery {
  ...
  async send(message: InvitationDeliveryMessage): Promise<'SENT' | 'FAILED'> {
    ...
    if ((stage === 'local' || stage === 'lab') && (!allowedRecipient || message.email.toLowerCase() !== allowedRecipient.toLowerCase())) {
      this.logger.warn({ invitationId: message.invitationId }, 'Destinatário fora do escopo de teste Resend')
      return 'FAILED'
    }
    let failureReason: 'TEMPORARY' | 'PERMANENT' | 'AMBIGUOUS' = 'AMBIGUOUS'
    try {
      if (await this.deliveries.isSuppressed(message.invitationId)) {
        await this.deliveries.recordFailure(message.invitationId, this.tokens.hash(message.token), 'PERMANENT', message.retryJobId)
        return 'FAILED'
      }
      ...
      if (!response.ok) {
        ...
        if ((response.status === 429 && name === 'rate_limit_exceeded') || (response.status === 503 && name === 'service_unavailable')) failureReason = 'TEMPORARY'
        else if (response.status >= 400 && response.status < 500 && response.status !== 409) failureReason = 'PERMANENT'
        throw new Error('RESEND_REJECTED')
      }
      const result: unknown = await response.json()
      if (!result || typeof result !== 'object' || !('id' in result) || typeof result.id !== 'string' || !result.id) throw new Error('RESEND_INVALID_RESPONSE')
      await this.deliveries.recordSent(message.invitationId, result.id, this.tokens.hash(message.token))
      // API acceptance is not confirmation of delivery to the mailbox.
      return 'SENT'
    } catch {
      await this.deliveries.recordFailure(message.invitationId, this.tokens.hash(message.token), failureReason, message.retryJobId).catch(() => {
