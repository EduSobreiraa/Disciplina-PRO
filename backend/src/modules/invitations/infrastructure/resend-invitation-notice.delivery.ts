...
export class ResendInvitationNoticeDelivery extends InvitationNoticeDelivery {
  ...
  async send(notice: InvitationNotice): Promise<'SENT' | 'FAILED' | 'BLOCKED' | 'UNCERTAIN'> {
    ...
    const body = JSON.stringify({
      ...
      text: `A entrega do convite ${notice.invitationId} precisa de revisão. O envio automático foi interrompido. Acesse o painel de administração para verificar; não reenvie sem confirmar a causa.\n${this.config.get('FRONTEND_URL', { infer: true })}`,
    })
    try {
