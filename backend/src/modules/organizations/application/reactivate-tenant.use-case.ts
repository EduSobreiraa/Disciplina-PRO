...
export class ReactivateTenantUseCase {
  ...
  execute(context: CurrentPlatformContext, input: { tenantId: string; reason: string }) {
    return this.tenants.reactivate({ tenantId: validateOrganizationId(input.tenantId), actorPlatformAccessId: context.platformAccessId, reason: normalizeOperationalReason(input.reason), now: new Date() })
  }
}
