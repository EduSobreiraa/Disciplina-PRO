...
async function executeCycle(
  processor: ProcessInternalEventsUseCase,
  emailRetries: ProcessInvitationRetriesUseCase,
  emailNotices: ProcessInvitationNoticesUseCase,
) {
...
async function runCycles(input: {
  processor: ProcessInternalEventsUseCase
  emailRetries: ProcessInvitationRetriesUseCase
  emailNotices: ProcessInvitationNoticesUseCase
  pollInterval: number
  ...
  while (!stopping) {
    try {
      await executeCycle(input.processor, input.emailRetries, input.emailNotices)
      if (!stopping) await sleep(input.pollInterval)
    } catch (error) {
      ...
      if (!stopping) await sleep(input.errorDelay)
    }
  }
}
...
async function runApplication() {
  ...
  try {
    ...
    await runCycles({
      processor: app.get(ProcessInternalEventsUseCase),
      emailRetries: app.get(ProcessInvitationRetriesUseCase),
      emailNotices: app.get(ProcessInvitationNoticesUseCase),
      pollInterval,
      errorDelay: config.get('OUTBOX_WORKER_ERROR_DELAY_MS', { infer: true }),
    })
  } catch (error) {
...
async function run() {
  while (!stopping) await runApplication()
}
