...
function transitionLabel(action) {
  if (action === 'suspend') return 'Suspender'
  return action === 'inactivate' ? 'Inativar' : 'Reativar'
}
...
export function MembershipAdministrationPanel({ administration }) {
  ...
  function allowedTransitions(membership) {
    ...
    if (administration.canManageTeams) return ['suspend', 'inactivate']
    return membership.role === 'USER' ? ['inactivate'] : []
  }
