...
export function InvitationAdministrationPanel({ administration }) {
  ...
  return <section className="admin-panel admin-invitations"><header><div><span>Entrada nominal</span><h2>Convites</h2></div></header>
    ...
    {administration.delivery && <p className={`admin-delivery ${administration.delivery.status?.toLowerCase()}`} role="status">Entrega para {administration.delivery.email}: {administration.delivery.status === 'SENT' ? 'aceita pelo provedor de e-mail; recebimento ainda não confirmado' : 'não confirmada; consulte os avisos de revisão antes de reenviar'}</p>}
    <InvitationDeliveryAlerts invitations={administration.invitations} />
