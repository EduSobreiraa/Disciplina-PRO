  ...
  return (
    <>
      ...
      {sections.map((section) => {
        ...
        return <section className="p66-check-group" key={section.key}><header><div><b>{section.icon}</b><span><strong>{section.title}</strong><small>{section.subtitle}</small></span></div><em>{sectionChecked}/{section.items.length}</em></header>{section.items.map((item, index) => { const key = itemKey(section.key, index); const checked = Boolean(checklist[key]); return <button disabled={!active || checked || savingKey === key} className={checked ? 'checked' : ''} type="button" key={item} onClick={() => toggleItem(section.key, index)}><i>{checked ? '✓' : ''}</i><span>{savingKey === key ? 'Salvando…' : item}</span></button> })}</section>
      })}
