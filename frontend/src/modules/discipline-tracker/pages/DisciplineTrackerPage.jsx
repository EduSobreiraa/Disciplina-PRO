...
import { createTrackerBackup, parseTrackerBackup } from '../services/tracker-backup'
...
export function DisciplineTrackerPage() {
  ...
  function exportBackup() {
    const blob = new Blob([JSON.stringify(createTrackerBackup(tracker.state), null, 2)], { type: 'application/json' })
  ...
  async function importBackup(event) {
    ...
    if (!file) return
    try {
      const data = parseTrackerBackup(await file.text())
      if (!window.confirm('Importar este backup substituirá os dados atuais do tracker. Continuar?')) return
      tracker.replaceState(data); setBackupMessage('Backup importado com sucesso.')
  ...
  return <>
    ...
    {justification && <JustificationDialog
      ...
      onSave={(text) => { tracker.saveJustification(justification.key, text); setJustification(null) }}
    />}
  </>
}
